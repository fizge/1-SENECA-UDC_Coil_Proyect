# DataPilot Internal Documentation
This documentation provides essential background information for the DataPilot project team, covering all foundational details about the
DataPilot software and its users. The purpose of this resource is to keep team members informed of the context that guides the project, and provide new team members background information on project goals, platforms used, the users, and methodology.
...
The purpose of DataPilot is to predict data from data sets. The data sets that are input into the application must be database, excel, pickle, or comma-separated value files. The output datasets can be saved and uploaded only as pickle files. 
...
### Audience
The intended audience of DataPilot are small to medium-scale business owners with low to average technical expertise who want to gain data-based insights that can inform their decisions. DataPilot can be used to make other industry-specific predictions by users in different professions, such as e-commerce and education.

DataPilot's priority is to create an intuitive tool that simplifies data processing, allowing users to quickly gain actionable insights. By focusing on accessibility and ease of use, we aim to empower our audience with the confidence to use data analytics without requiring advanced technical skills or training. While our example below is an example of a target user, always consider the diverse technical backgrounds of users and prioritize features that streamline processes, minimize confusion, and provide clear, impactful insights.

<details>
<summary>View DataPilot’s Target User</summary>

This chart shows the traits, motivations, and pain points of DataPilot's target user:

| User Trait | Description |
| --- | --- |
| Age | 20 + |
| Occupation | Small to medium-scale business owners. |
| Technical Skill Level |  DataPilot users have varying levels of technical expertise, ranging from beginners unfamiliar with data analysis tools to more experienced users seeking a straightforward way to predict profits and make data-driven decisions. |
...
| Expectations | Empathetic and concise instructions that are labelled,  the user’s existing data should be fully compatible with DataPilot’s upload feature. They expect to understand how to use correct input that will result in helpful and informative output results to support their goals. |

</details>

### Platforms and Technology 
DataPilot was created using a variety of tools and platforms to create a simple and user-friendly application. <br>
...
The dataset formats that were used to create DataPilot were:
- Database files (.db)
- Excel files (.xls)
- Pickle files (.pkl)
- CSV files (.csv)
  **Note:** DataPilot can only be used with these data file formats.
...
4.  Sprint 4: The documentation team worked on creating the preliminary README and GitHub document. The developer team worked on creating linear models for prediction using the GUI, and improving DataPilot's graphical interface.
...
6.  Sprint 6: The developers further worked on the file selection feature, saving features, and creating a polished user interface for DataPilot. The documentation team worked on revising the README by including DataPilot user interface images.
...
Understanding these concepts is integral to understanding how DataPilot functions. 
...
### How does DataPilot use Linear Regression and Machine Learning? 
DataPilot allows users to effortlessly create and work with linear regression models, helping businesses predict profits using past data. By uploading an existing spreadsheet to DataPilot, users can quickly generate a linear regression model, while AI enhances the results by delivering actionable insights. This process requires minimal effort and no advanced technical knowledge, making data-driven predictions accessible to everyone.

## Project Specific Information
This section explains who DataPilot was created by, their role in the project, and the role of the documentation team. <br>
