...
class DataProcessing:
    ...
    def display_data_in_treeview(self, data):
        self.app.v.geometry("1000x550+0+0")
    ...
    def options_selection(self, columns):
        ...
        self.generate_button = ctk.CTkButton(self.option_frame, text="Generate model", font=("Arial", 24, "bold"), height=70, width=150, command=self.modeling.generate_model)

    def update_output_options(self, selected_input):
        if selected_input:
            self.output_columns = [col for col in self.input_columns if col != selected_input]
    ...
    def update_input_options(self, selected_output):
        if selected_output:
            self.input_columns = [col for col in self.output_columns if col != selected_output]
    ...
    def confirm_selections(self):
        ...
        self.option_frame.grid(row=6, column=0, columnspan=2, pady=(10, 20), padx=10, sticky="ew")
        self.preprocess_label.grid(row=2, column=0, columnspan=4, padx=100, pady=10, sticky="nsew")
        self.remove_nan_button.grid(row=3, column=0, padx=(100, 0), pady=10, sticky="ew")
        ...
        self.fill_constant_button.grid(row=3, column=3, padx=(20, 0), pady=10, sticky="ew")
